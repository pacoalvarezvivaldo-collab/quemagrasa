...
- **Rediseño SPORTIUUM (marca, nav inferior, anillo en Rápido):** cerrado, mergeado a `main` y pusheado (`15382e0`), sin bloqueantes. 3 rondas de review de rama (rango completo + 2 de seguimiento), 4 bugs encontrados y corregidos. Pendientes de pulido abajo.

## Rediseño SPORTIUUM — resumen (mergeado)

- **Spec:** `docs/superpowers/specs/2026-09-02-rediseno-sportiuum-design.md` (aprobada por el usuario).
- **Plan:** `docs/superpowers/plans/2026-09-02-rediseno-sportiuum.md` — 10 tareas, todas implementadas y con su revisión de tarea aprobada (ledger quedó en `.superpowers/sdd/progress.md` del worktree, ya eliminado tras el merge).
- **Hallazgos bloqueantes del review final — todos corregidos:**
  ...
  - [x] ~~`correr.html` — mismo bug de solapamiento que el follow-pill, pero en el toast de "compartir" de la pantalla resumen~~ — resuelto (`b8f64ba`): mismo ajuste `+64px` en `bottom`.
  - [x] ~~`plan-engine.js` — la llamada nueva a `Nav.hide()`/`Nav.render()` no seguía la convención del archivo de guardar globals opcionales (`window.Streak`, `window.Prefs`)~~ — resuelto (`b8f64ba`): envuelta en `if(window.Nav)`.
- **Verificado manualmente en navegador** (servidor estático local): nav inferior correcta en las 9 páginas, se oculta durante sesión activa de Rápido/Cardio/Correr, tarjeta "Correr / Caminar" funciona desde `cardio.html`, anillo circular de `rapido.html` se llena en sincronía con el reloj. Los 4 fixes de review (nav en Casa/Gimnasio, offset del follow-pill y del toast de Correr) se verificaron con `node --check` pero no a mano en navegador — bajo riesgo, son cambios de offset CSS y un guard.
- Commits: `4fef64a`(nav.js) → `3fc5897`(entrenar.html) → `79574bc`(index.html) → `d31cb3a`(entrenamientos+gimnasio) → `71f89e5`(cardio) → `01f9fa7`(correr) → `fc085f0`(rapido nav) → `1ceb4cb`(rapido anillo) → `016d0cc`(estadisticas+ajustes) → `1a002cf`,`6732fe9`,`b8f64ba`(fixes de review) → merge `15382e0` a `main`.
...
- [ ] **`+64px` de altura del nav bar hardcodeado dos veces** (`cardio.html`'s follow-pill, `correr.html`'s toast) en vez de derivarse de `NAV_PADDING` en `nav.js:35` — si esa altura cambia algún día, los dos offsets quedan desactualizados en silencio.
- [ ] **`.follow-pill` (z-index:60) por debajo del nav bar (z-index:80)** — si la altura real del nav llegara a superar los 64px asumidos (fuente más grande del sistema, label que rompe línea), el nav taparía el pill en vez de que el pill flote arriba. Hipotético, no reproducido.
