...
- **Qué falta:** review final de rama completa (rango `c22acec..HEAD`, 8 agentes en paralelo) ya corrió y encontró 2 hallazgos bloqueantes — ver abajo, en corrección. Tras corregirlos y confirmar `node --check` limpio, mergear con `superpowers:finishing-a-development-branch`.
- **Hallazgos bloqueantes del review final (en corrección):**
  - `plan-engine.js` — `showScreen()` nunca llama `Nav.hide()`/`Nav.render()` al entrar/salir de la pantalla de sesión activa ("player") usada por `entrenamientos.html`/`gimnasio.html`. El nav queda visible y tocable durante el entrenamiento (a diferencia de `rapido.html`/`correr.html`/`cardio.html`, que sí lo ocultan), riesgo de tocarlo por accidente y perder `currentIdx` (no se persiste, se resetea a 0 al reabrir el día).
  - `cardio.html` — el "follow-pill" (botón para volver al bloque actual, z-index 60) queda tapado detrás del nav bar nuevo (`#sp-nav`, z-index 80) durante cardio en modo normal (no gym, que sí llama `Nav.hide()`).
