...
- **`rapido.html`**: ya tiene el mismo beep de aviso 10s antes de terminar el descanso entre ejercicios (`tick()`, línea ~259) — pasa a respetar `soundWarningOn` igual que `plan-engine.js`. Su Wake Lock, hoy siempre activo sin opción de apagarlo, pasa a respetar `keepScreenOn`.
...
- **`cardio.html`**: su toggle "Mantener pantalla encendida" (`bgMode`) migra a leer/escribir la llave compartida `keepScreenOn`. Su toggle "Aviso 10s antes del sprint" (`preWarn` — el mismo mecanismo de aviso+vibración que los demás modos, solo que aquí específico de los bloques sprint) migra a `soundWarningOn`. Cambiarlos ahí también los cambia en el resto de la app y viceversa. Su toggle "Beeps de cambio de bloque" (`soundOn`) **no se toca** — es un mute general de todos los beeps de Cardio (incluye el conteo regresivo al final de cada intervalo, algo que no existe en los otros modos), no tiene equivalente limpio fuera de Cardio y se queda como ajuste propio de esa pantalla. Sus otros ajustes (números gigantes ⛶, ajuste ± en vivo, "propagar a bloques siguientes", calibración de máximos de la máquina) tampoco se tocan.
...
| `rapido.html` | gatea el beep existente antes de terminar el descanso | gatea el Wake Lock existente (hoy incondicional) | primer toque |
| `correr.html` | no aplica | gatea el Wake Lock existente (hoy incondicional) | primer toque |
| `cardio.html` | migra `preWarn` a la llave compartida (`soundOn` se queda propio de Cardio) | migra `bgMode` a la llave compartida | primer toque |
